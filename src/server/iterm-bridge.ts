...
export class ItermBridge implements TerminalBridge {
  ...
  private static escapeForAppleScript(text: string): string {
    return text.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
  }

  private static async runAppleScript(script: string): Promise<string> {
    try {
  ...
  async enumerateSessions(): Promise<ITermSession[]> {
    ...
    try {
      const result = await ItermBridge.runAppleScript(script);
      if (!result) return [];
  ...
  async getSessionContent(sessionId: string): Promise<string> {
    ...
    try {
      return await ItermBridge.runAppleScript(script);
    } catch {
      return '';
    }
  }
  ...
  async sendInput(sessionId: string, text: string): Promise<void> {
    ...
    await ItermBridge.runAppleScript(script);
  }
